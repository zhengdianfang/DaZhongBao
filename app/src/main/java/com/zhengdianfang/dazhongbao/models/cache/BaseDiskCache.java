 ...
 */
public class BaseDiskCache implements Cache {
     ...
     */
    public BaseDiskCache(File rootDirectory, long maxCacheSizeInBytes) {
     ...
     * @param rootDirectory The root directory of the cache.
     */
    public BaseDiskCache(File rootDirectory) {
    ...
    public synchronized Entry get(String key) {
        ...
        try {
            cis = new CountingInputStream(new Base64InputStream(new FileInputStream(file), Base64.DEFAULT));
    ...
    public synchronized void initialize() {
        ...
        for (File file : files) {
            ...
            try {
                fis = new BufferedInputStream(new Base64InputStream(new FileInputStream(file), Base64.DEFAULT));
    ...
    public synchronized void put(String key, Entry entry) {
        ...
        File file = getFileForKey(key);
        try {
            Base64OutputStream fos = new Base64OutputStream(new FileOutputStream(file), Base64.DEFAULT);
    ...
    public synchronized void remove(String key) {
        ...
        removeEntry(key);
        if (!deleted) {
            Log.d(TAG, "Could not delete cache entry for key=" + key + ", filename=" + getFilenameForKey(key));
        }
    }
    ...
    private void pruneIfNeeded(int neededSpace) {
        ...
        Log.v(TAG, "pruned " + prunedFiles +" files, " +(mTotalSize - before) + " bytes, "+ (SystemClock.elapsedRealtime() - startTime) + " ms");
    }
